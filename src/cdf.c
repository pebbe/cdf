    ...
    for (int varid = 0; varid < nvars; varid++) {
        ...
        for (int attnum = 0;; attnum++) {
            ...
            if (xtype == NC_CHAR) {
                ...
                XXnc(nc_inq_attlen(ncid, varid, name, &len));
                char *v[len];
                memset(v, 0, len * sizeof(char *));
                XXnc(nc_get_att_string(ncid, varid, name, v));
                if (len == 1)
                    printf("\"%s\"", quote(v[0]));
                else {
                    printf("null");
                }
                XXnc(nc_free_string(len, v));
        ...
        if (xtype == NC_DOUBLE)
            doDouble(ncid, varid, ndims);
        else if (xtype == NC_STRING)
            doString(ncid, varid, ndims);
